CREATE OR REPLACE FUNCTION public.seed_media_snapshot_for_batch(_batch_id uuid, _media_key text, _source text, _external_id text, _media_type text, _title text, _title_original text DEFAULT NULL::text, _poster_url text DEFAULT NULL::text, _backdrop_url text DEFAULT NULL::text, _release_date text DEFAULT NULL::text, _genres text[] DEFAULT '{}'::text[], _platforms jsonb DEFAULT '[]'::jsonb, _score numeric DEFAULT NULL::numeric)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Validations d'entrée (miroir de seed_media_snapshot)
  ...
  -- LIAISON FORTE : _media_key doit correspondre à un import_item réel du batch,
  -- avec matched_media_key issu du matching côté serveur (Preview). Cela borne
  -- naturellement le nombre de seeds au nombre d'items du batch et interdit
  -- l'usage du batch comme jeton général pour semer des médias arbitraires.
  SELECT EXISTS (
    SELECT 1 FROM public.import_items
    WHERE batch_id = _batch_id
      AND user_id = caller
      AND matched_media_key = canonical_key
  ) INTO item_exists;
  IF NOT item_exists THEN
    RAISE EXCEPTION 'Média non lié à un élément prévisualisé de ce batch.';
  END IF;

  -- Plafond produit absolu (ceinture + bretelles). Compte les rows créées par
  -- ce membre via un item de ce batch.
  ...
  -- Idempotent : ON CONFLICT DO NOTHING. Un item déjà seedé ne recrée rien.
